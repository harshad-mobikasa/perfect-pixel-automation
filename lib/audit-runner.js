...
export async function runAudit(jobId, auditRequest, options = {}) {
  ...
  try {
    ...
    const reportFiles = await nameReportFiles(
      await findReportFiles(workDir, project.projectDir, auditRequest.suite),
      job,
      auditRequest.suite,
    )
    ...
    if (persistReports) {
      ...
      await notifyAuditResult(jobStore, jobId)
      return
    }
...
async function findReportFiles(workDir, projectDir, suite) {
  ...
  let pdfs = await collectPdfFiles(dir)

  if (pdfs.length === 0) {
    const resultsDir = join(workDir, 'qa-results')
    pdfs = (await collectPdfFiles(resultsDir)).filter((file) => {
      const normalizedPath = file.path.replaceAll('\\', '/').toLowerCase()
      const normalizedSuite = suite.toLowerCase()
      return file.fileName.toLowerCase().includes(normalizedSuite) || normalizedPath.includes(`/reports/${normalizedSuite}/`)
    })
  }

  pdfs.sort((left, right) => left.path.localeCompare(right.path))
...
async function collectPdfFiles(dir) {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      files.push(...(await collectPdfFiles(path)))
...
function formatReportTimestamp(value) {
  ...
  if (Number.isNaN(date.getTime())) return formatReportTimestamp(Date.now())
...
function reportVariant(fileName, suite) {
  const baseName = fileName.replace(/\.pdf$/i, '')
  const withoutSuite = baseName.replace(new RegExp(`^${suite}[-_]?audit[-_]?report[-_]?`, 'i'), '')
  return sanitizeFileName(withoutSuite || baseName || 'report')
}

async function nameReportFiles(reportFiles, job, suite) {
  let projectName = ''
  if (job?.projectId) {
  ...
  const userSlug = sanitizeFileName(job?.createdBy?.name || job?.createdBy?.email?.split('@')[0] || 'user')
  const projectSlug = sanitizeFileName(projectName || getProjectInfo(job?.request?.url ?? 'https://audit.local', job?.id ?? 'audit').projectName)
  const suiteSlug = sanitizeFileName(suite || 'audit')
  const timestamp = formatReportTimestamp(job?.createdAt ?? Date.now())

  return reportFiles.map((file) => ({
    ...file,
    fileName: `${userSlug}-${projectSlug}-${suiteSlug}-${timestamp}-${reportVariant(file.fileName, suiteSlug)}.pdf`,
  }))
}

async function notifyAuditResult(jobStore, jobId) {
  const job = await jobStore.getJob(jobId)
  const recipient = job?.createdBy?.email
  if (!recipient || (job.status !== 'done' && job.status !== 'failed')) return
  ...
  try {
    ...
    await jobStore.updateJob(jobId, {
      emailNotificationSentAt: Date.now(),
      emailNotificationError: null,
      updatedAt: Date.now(),
    })
  } catch (error) {
    await jobStore.updateJob(jobId, {
      emailNotificationAttemptedAt: Date.now(),
      emailNotificationError: error instanceof Error ? error.message.slice(0, 300) : 'Email notification failed',
      updatedAt: Date.now(),
    })
  }
}
