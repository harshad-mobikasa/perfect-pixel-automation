...
async function createInvitedUser(actor, input) {
  ...
  const { password, generated } = resolveAccountPassword(input.password)
  ...
  if (requestedRole === ROLE.ADMIN) {
    ...
    return { user: publicUser(user), temporaryPassword: generated ? password : null }
  }
  ...
  return {
    user: publicUser(await store.getUserById(user.id)),
...
async function inviteProjectMember(actor, projectId, input) {
  ...
  if (!user) {
    ...
    created = true
    temporaryPassword = resolved.generated ? resolved.password : null
  ...
  return { user: publicUser(saved), created, temporaryPassword }
}
...
async function assignGeneratedPassword(actor, userId) {
  ...
  return { user: publicUser(target), temporaryPassword: password }
}
