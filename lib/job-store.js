...
const WORKER_HEARTBEAT_KEY = 'audit:worker:heartbeat'
...
class MemoryJobStore {
  ...
  async ensureQueuedJob(_jobId) {
    return false
  }

  async recordWorkerHeartbeat(_workerId) {
    return { workerId: 'local', at: Date.now() }
  }
  ...
  async listPendingQueue() {
    const ids = []
    for (const [id, job] of this.jobMap.entries()) {
      if (job.status === 'queued') ids.push(id)
    }
    return ids
  }

  async getQueuePosition(jobId) {
    const live = await this.listPendingQueue()
...
class RedisJobStore {
  ...
  async ensureQueuedJob(jobId) {
    const existing = await this.getJob(jobId)
    if (existing?.status !== 'queued' || !existing.request) return false

    const position = await this.command(['LPOS', QUEUE_KEY, jobId])
    if (position !== null) return false

    await this.command(['RPUSH', QUEUE_KEY, jobId])
    return true
  }
  ...
  async getWorkerHeartbeat() {
    const value = await this.command(['GET', WORKER_HEARTBEAT_KEY])
  ...
  async recoverStaleRunningJobs() {
    ...
    for (const jobId of ids) {
      ...
      if (Date.now() - (job.updatedAt ?? job.startedAt ?? job.createdAt ?? 0) < STALE_RUNNING_MS) {
        continue
      }
      ...
      const next = {
        ...
        stage: 'Queued',
        progress: 5,
        lastMessage: 'Previous worker stopped before finishing; queued again for the next worker',
      }
      const commands = [
        ['SET', this.jobKey(jobId), JSON.stringify(next), 'EX', `${JOB_TTL_SECONDS}`],
        ['SREM', PROCESSING_KEY, jobId],
      ...
      const position = await this.command(['LPOS', QUEUE_KEY, jobId])
      if (position === null) commands.push(['RPUSH', QUEUE_KEY, jobId])
      await this.pipeline(commands)
      recovered += 1
    }
