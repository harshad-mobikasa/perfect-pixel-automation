...
function cleanAuditError(error) {
  const raw = String(error || '').trim()
  if (!raw) return 'The audit failed while generating the report.'

  const firstLine = raw.split(/\r?\n/).find((line) => line.trim())?.trim() ?? ''
  if (/command failed/i.test(firstLine)) {
    return 'The audit runner failed while generating the PDF report.'
  }
...
function textForAuditResult({ name, projectName, suite, status, reportFiles, error, jobId }) {
  ...
  if (status === 'done') {
    ...
    lines.push('', `Error summary: ${cleanAuditError(error)}`)
    lines.push('Open the audit tool to review the full job status.')
  }
